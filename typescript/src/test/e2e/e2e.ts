...
describe('end to end system tests', () => {
  ...
  describe('e2e naive w/ v1 (legacy) yaml', () => {
    it('should get sample json and convert it to csv', () => {
      ...
      return jiraExtractor.getWorkItems(jiraSettings, ()=>{}).then((workItems) => {
        const actualCsv = jiraExtractor.toCSV(workItems, jiraSettings.Stages, jiraSettings.Attributes);
  ...
  describe('e2e naive w/ v1 yaml', () => {
    it('should get sample json and convert it to csv correctly', () => {
      ...
      const jiraExtractor = new JiraExtractor();
      return jiraExtractor.getWorkItems(jiraSettings, ()=>{}).then(workItems => {
        const csvData = jiraExtractor.toCSV(workItems, jiraSettings.Stages, jiraSettings.Attributes);
